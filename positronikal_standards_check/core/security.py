...
class SecurityValidator:
                    ...
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"Security file exists but appears minimal: {filepath}"
                            ),
                        }
                    )
            else:
    ...
    def _check_sensitive_data(self) -> List[Dict]:
        ...
        if found_issues:
            ...
            issue_msgs = [
                f"{issue['file']} ({issue['type']})" for issue in found_issues[:3]
            ]
            results.append(
                {
                    ...
                    "status": "fail",
                    "message": (
                        f"Potential sensitive data found in: "
                        f"{', '.join(issue_msgs)}"
                        f"{'...' if len(found_issues) > 3 else ''}"
                    ),
                }
            )
        else:
    ...
    def _check_commit_signing(self) -> List[Dict]:
        ...
        try:
            ...
            result = subprocess.run(
                ["git", "log", "--pretty=format:%H %G?", "-10"],  # noqa: S607
                capture_output=True,
                text=True,
                cwd=self.repo_path,
            )
                    ...
                    results.append(
                        {
                            ...
                            "status": "pass",
                            "message": (
                                f"All recent commits are signed "
                                f"({signed_count}/{signed_count})"
                            ),
                        }
                    )
                elif signed_count > 0:
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"Some commits are unsigned "
                                f"({unsigned_count}/{signed_count + unsigned_count})"
                            ),
                        }
                    )
                else:
                ...
                results.append(
                    {
                        ...
                        "status": "warning",
                        "message": (
                            "Could not check commit signing (not a git repository?)"
                        ),
                    }
                )
            ...
            try:
                ...
                if "permissions:" in content:
                    if (
                        ...
                        or "permissions: read-all" in content
                    ):
                        results.append(
                            {
                                ...
                                "status": "pass",
                                "message": (
                                    f"Workflow {workflow_file.name} has "
                                    f"restricted permissions"
                                ),
                            }
                        )
                    else:
                        results.append(
                            {
                                ...
                                "status": "warning",
                                "message": (
                                    f"Workflow {workflow_file.name} may have "
                                    f"excessive permissions"
                                ),
                            }
                        )
                else:
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"Workflow {workflow_file.name} does not "
                                f"explicitly set permissions"
                            ),
                        }
                    )
                    ...
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"Workflow {workflow_file.name} uses custom secrets - "
                                f"ensure they are properly secured"
                            ),
                        }
                    )
    ...
    def _run_sast_tools(self) -> List[Dict]:
        ...
        for language in detected_languages:
            if language in self.SAST_TOOLS:
                ...
                try:
                    # noqa justification: tool_cmd comes from the
                    # class-defined SAST_TOOLS mapping, not user input.
                        ...
                        results.append(
                            {
                                ...
                                "status": "fail",
                                "message": (
                                    f"{language.capitalize()} SAST found issues: "
                                    f"{output[:200]}..."
                                ),
                            }
                        )
                    ...
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"{language.capitalize()} SAST tool not "
                                f"available: {tool_cmd[0]}"
                            ),
                        }
                    )
