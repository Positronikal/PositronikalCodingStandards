...
class CodeStandardsValidator:
            ...
            results.append(
                {
                    ...
                    "status": "fail",
                    "message": (
                        f"Files with Windows line endings (CRLF): "
                        f"{', '.join(files_with_crlf[:5])}"
                        f"{'...' if len(files_with_crlf) > 5 else ''}"
                    ),
                }
            )
            ...
            results.append(
                {
                    ...
                    "status": "pass",
                    "message": (
                        f"All {files_checked} source files use Unix line endings (LF)"
                    ),
                }
            )
            ...
            results.append(
                {
                    ...
                    "status": "fail",
                    "message": (
                        f"Files with non-UTF-8 encoding: "
                        f"{', '.join(files_with_issues[:5])}"
                        f"{'...' if len(files_with_issues) > 5 else ''}"
                    ),
                }
            )
        elif files_checked > 0:
    ...
    def _check_line_length(self) -> List[Dict]:
        ...
        if violations:
            ...
            violation_msgs = [
                f"{v['file']}:{v['line']} ({v['length']} chars)" for v in violations[:5]
            ]
            results.append(
                {
                    "check": "line_length",
            ...
            results.append(
                {
                    ...
                    "status": "pass",
                    "message": (
                        f"All lines in {files_checked} source files within "
                        f"{self.MAX_LINE_LENGTH} character limit"
                    ),
                }
            )
            ...
            results.append(
                {
                    ...
                    "status": "fail",
                    "message": (
                        f"Files with tab indentation: "
                        f"{', '.join(files_with_tabs[:5])}"
                        f"{'...' if len(files_with_tabs) > 5 else ''}"
                    ),
                }
            )
        elif files_checked > 0:
            results.append(
                {
                    ...
                    "status": "pass",
                    "message": (
                        f"All {files_checked} source files use space indentation"
                    ),
                }
            )
    ...
    def _check_trailing_whitespace(self) -> List[Dict]:
        ...
        for file_path in self._get_source_files():
            ...
            try:
                ...
                logger.warning(
                    f"Could not check trailing whitespace in {file_path}: {e}"
                )
            ...
            results.append(
                {
                    ...
                    "status": "fail",
                    "message": (
                        f"Files with trailing whitespace: "
                        f"{', '.join(files_with_trailing[:5])}"
                        f"{'...' if len(files_with_trailing) > 5 else ''}"
                    ),
                }
            )
        elif files_checked > 0:
            results.append(
                {
                    ...
                    "status": "pass",
                    "message": (
                        f"No trailing whitespace found in {files_checked} source files"
                    ),
                }
            )
        ...
        for language in detected_languages:
            if language in self.LINTERS:
                ...
                try:
                    ...
                    # class-defined LINTERS mapping, not user input.
                        ...
                        results.append(
                            {
                                ...
                                "status": "fail",
                                "message": (
                                    f"{language.capitalize()} linter found issues: "
                                    f"{output[:200]}..."
                                ),
                            }
                        )
                    ...
                    results.append(
                        {
                            ...
                            "status": "warning",
                            "message": (
                                f"{language.capitalize()} linter not available: "
                                f"{linter_cmd[0]}"
                            ),
                        }
                    )
